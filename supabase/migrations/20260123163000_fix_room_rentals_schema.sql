-- Fix Room Rentals Table Schema
-- The application expects several columns that appear to be missing.
-- We add them safely using IF NOT EXISTS.
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'room_rentals' AND column_name = 'renter_email') THEN
        ALTER TABLE public.room_rentals ADD COLUMN renter_email TEXT;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'room_rentals' AND column_name = 'renter_phone') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'room_rentals' AND column_name = 'total_price') THEN
    ...
    -- Fix: Drop update_updated_at_column if it exists for this table to avoid conflicts or re-create it?
    -- Usually fine.
END $$;
...
-- Policy: Doctors can update their own rentals (optional, for cancellation)
DROP POLICY IF EXISTS "Doctors can update own rentals" ON public.room_rentals;
CREATE POLICY "Doctors can update own rentals"
ON public.room_rentals FOR UPDATE
TO authenticated
